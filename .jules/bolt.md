...
## 2024-05-24 - Resolve Claude Image References Concurrently
**Learning:** Sequential `await` in loops over prompt references (especially images) adds significant linear latency, scaling with the number of references.
**Action:** Use `Promise.all` over mapped references to fetch binary data concurrently before looping sequentially for state-dependent prompt mutation.
