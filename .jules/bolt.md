...
## 2024-03-14 - Optimize sequential context collection via Promise.all
**Learning:** Sequential async file system accesses (like checking presence of multiple `package.json` across workspace folders) in `promptWorkspaceLabels` cause an N+1 query issue, drastically slowing down label loading.
**Action:** When mapping over independent keys and array elements representing file IO tasks, refactor standard `for...of` async loop into parallel execution using `Promise.all` + `Array.map()` to maximize speed.
