...
import {
	...
	Props as DefinitionAroundCursorProps,
	determineNodeToDocument,
	NodeToDocument,
			...
			*/
			...
			replyInterpreter = new InlineReplyInterpreter(
				...
				(line) =>
					!line.value.includes(
						'FILEPATH',
					) /* @ulugbekna: this's to remove marker lines if any */ &&
					!linesInOriginalRange.has(
						line.value.trim(),
					) /* @ulugbekna: this's to prevent repeating of existing code */,
			);
		} else {
			replyInterpreter = new InlineReplyInterpreter(
				this.context.document.uri,
...
class DocPrompt extends PromptElement<Props> {
	override render(
		...
		const language = languageIdToMDCodeBlockLang(
			this.props.documentContext.language.languageId,
		);

		const rewrittenMessage =
			this.props.chatVariables.substituteVariablesWithReferences(
		...
		return (
			<>
				<SystemMessage>
					...
					<br />
					For questions not related to software development, you<br />
					should give a reminder that you are an AI programming<br />
					assistant.<br />
					<br />
				</SystemMessage>
				<ChatToolReferences
				...
				>
					<InstructionMessage>
						When user asks you to document something, you must
						answer in the form of a {language} markdown code block.
						<br />
					</InstructionMessage>
				</HistoryWithInstructions>
