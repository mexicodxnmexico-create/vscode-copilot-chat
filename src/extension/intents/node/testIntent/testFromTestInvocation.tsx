...
export class TestFromTestInvocation implements IIntentInvocation {
	...
	constructor(
		...
		@ITestGenInfoStorage
		private readonly testGenInfoStorage: ITestGenInfoStorage,
	) {}
...
class TestFromTestPrompt extends PromptElement<Props> {
	...
	override async render(_state: void, sizing: PromptSizing) {
		...
		const { context, testGenInfo, alreadyConsumedChatVariable } =
			this.props;
		...
		if (testGenInfo !== undefined) {
			const srcFileDoc = await this.workspaceService.openTextDocument(
				testGenInfo.uri,
			);
			const declStart = testGenInfo.target.start;
			const expandedRange = testGenInfo.target.with(
				declStart.with(declStart.line, 0),
			);
			...
			<>
				...
				>
					<InstructionMessage priority={1000}>
						The user has a {context.language.languageId} file opened<br />
						in a code editor.<br />
						<br />
						The user includes some code snippets from the file.
						<br />
						Answer with a single {context.language.languageId} code<br />
						block.<br />
						<br />
						Your expertise is strictly limited to software<br />
				...
				<UserMessage>
					...
					{testGenInfo !== undefined &&
						testedDeclarationExcerpt !==
							undefined /* FIXME@ulugbekna: include class around */ && (
