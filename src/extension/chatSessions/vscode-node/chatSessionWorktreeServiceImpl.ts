...
export class ChatSessionWorktreeService extends Disposable implements IChatSessionWorktreeService {
	...
	async getWorktreeChanges(sessionId: string): Promise<readonly ChatSessionWorktreeFile[] | undefined> {
		...
		if (worktreeProperties.version === 1 && worktreeProperties.autoCommit === false) {
			...
			// Concurrently map over changes to fetch file stats, eliminating sequential await latency
			const changePromises = [...worktreeRepository.changes.indexChanges, ...worktreeRepository.changes.workingTree].map(async change => {
				try {
			...
			const changes = (await Promise.all(changePromises)).filter((c): c is ChatSessionWorktreeFile => c !== undefined);
