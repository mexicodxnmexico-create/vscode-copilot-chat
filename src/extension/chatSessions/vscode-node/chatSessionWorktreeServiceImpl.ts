...
export class ChatSessionWorktreeService extends Disposable implements IChatSessionWorktreeService {
	...
	async getWorktreeChanges(sessionId: string): Promise<readonly ChatSessionWorktreeFile[] | undefined> {
		...
		if (worktreeProperties.version === 1 && worktreeProperties.autoCommit === false) {
			...
			const changesPromises = [...worktreeRepository.changes.indexChanges, ...worktreeRepository.changes.workingTree].map(async (change) => {
				try {
			...
			const changesResult = await Promise.all(changesPromises);
			const changes: ChatSessionWorktreeFile[] = changesResult.filter((c): c is ChatSessionWorktreeFile => c !== undefined);
