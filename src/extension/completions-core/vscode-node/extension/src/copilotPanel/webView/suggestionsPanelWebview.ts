...
function handleSolutionUpdate(message: Message) {
	...
	if (solutionsContainer) {
		...
		// Security concern: prevent mutation XSS by avoiding piecemeal sanitization and concatenation.
		// Instead, assemble the complete HTML string and sanitize the entire payload at once.
		solutionsContainer.innerHTML = DOMPurify.sanitize(rawHtml, {
			ADD_TAGS: ['vscode-button'],
			ADD_ATTR: ['target', 'aria-label', 'appearance', 'data-solution-index', 'role']
		});
