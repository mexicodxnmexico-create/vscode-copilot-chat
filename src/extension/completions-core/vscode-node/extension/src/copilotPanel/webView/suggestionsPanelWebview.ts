			...
			.map((solution, index) => {
				const renderedCitation = solution.citation
					? `<p>
						...
						${sanitize(solution.citation.message)}
						<a href="${sanitize(solution.citation.url)}" target="_blank" rel="noopener noreferrer">Inspect source code</a>
					  </p>`
					: '';
				const sanitizedSnippet = sanitize(solution.htmlSnippet);
