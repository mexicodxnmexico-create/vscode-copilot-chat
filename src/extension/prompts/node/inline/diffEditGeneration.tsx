	...
	render() {
		return (
			<>
				...
				<br />
				Describe in a single sentence how you would solve the problem.<br />
				After that sentence, add an empty line. Then add a code block<br />
				with the fix.<br />
				<br />
				When proposing a change in the code, use a single code block<br />
				that starts with ```diff and that describes the changes in the<br />
				diff format. In the diff, always use tab to indent, never<br />
				spaces. Make sure that the diff format is valid and contains all<br />
				changes: Removed and unchanged lines must match exactly the<br />
				original code line. Keep the changes minimal and the diff short.<br />
				<br />
				When proposing to fix the problem by running a terminal command,<br />
				provide the terminal script in a code block that starts with<br />
				```bash.<br />
				<br />
			</>
		);
...
export class EditGenerationExampleSolution extends PromptElement {
	render() {
		return (
			<>
				The problem is that the class 'C' does not have a field or<br />
				property named 'Field'. To fix this, you need to add a 'Field'<br />
				property to the 'C' class.<br />
				<br />
				<br />
				```diff
				<br />
