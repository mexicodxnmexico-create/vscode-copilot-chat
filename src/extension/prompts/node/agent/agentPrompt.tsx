...
export class AgentPrompt extends PromptElement<AgentPromptProps> {
	...
	async render(state: void, sizing: PromptSizing) {
		...
		const baseAgentInstructions = (
			<>
				<SystemMessage>
					You are an expert AI programming assistant, working with a<br />
					user in the VS Code editor.<br />
					<br />
		...
		if (this.props.promptContext.modeInstructions) {
			...
			customInstructionsBodyParts.push(
				<Tag name="modeInstructions">
					You are currently running in "{name}" mode. Below are your<br />
					instructions for this mode, they must take precedence over<br />
					any instructions above.<br />
					<br />
					<br />
					{resolvedContent}
				</Tag>,
	...
	async render() {
		...
		return (
			<Tag name="additional_skills_reminder">
				...
				use the {ToolName.ReadFile} tool to read the corresponding<br />
				SKILL.md files. Multiple skill files may be needed for a single<br />
				request. These files contain best practices built from testing<br />
				that are needed for high-quality outputs.<br />
				<br />
			</Tag>
		);
	}
								...
								>
									...
									{isActive && (
										<>
											...
											can use the{' '}
											{ToolName.CoreGetTaskOutput} tool to<br />
											view its output.)<br />
										</>
									)}
								</Tag>
	...
	async render(state: void, sizing: PromptSizing) {
		...
		return (
			<>
				...
				{sections.join('\n')}
				<br />
				So be sure to check the current file contents before making any<br />
				new edits.<br />
			</>
		);
	}
