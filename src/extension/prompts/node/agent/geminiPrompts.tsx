		...
		return (
			<InstructionMessage>
				<Tag name="instructions">
					...
					left off.<br />
					<br />
					When a tool call is intended, you MUST actually invoke the<br />
					tool rather than describing or simulating the call in text.<br />
					Never write out a tool call as prose—use the provided<br />
					tool-calling mechanism directly.<br />
					<br />
		...
		return (
			<InstructionMessage>
				<Tag name="instructions">
					...
					{tools[ToolName.ReadFile] && (
						<>
							Use {ToolName.ReadFile} to read more context if<br />
							needed. Never pass the omitted line marker to an<br />
							edit tool.<br />
						</>
					)}
					<br />
					...
					Call tools repeatedly to take actions or gather context<br />
					until you have completed the task fully.<br />
					<br />
					...
					context.<br />
					<br />
					Provide updates to the user as you work. Explain what you<br />
					are doing and why before using tools. Be conversational and<br />
					helpful.<br />
				</Tag>
				<Tag name="toolUseInstructions">
					If the user is requesting a code sample, you can answer it<br />
					...
					{tools[ToolName.ReadFile] && (
						<>
							When using {ToolName.ReadFile}, prefer reading a<br />
							large section over calling it many times. Read large<br />
							enough context to ensure you get what you need.<br />
							<br />
						</>
					)}
					...
					{tools[ToolName.FindTextInFiles] && (
						<>
							Use {ToolName.FindTextInFiles} to get an overview of<br />
							a file by searching for a string within that one<br />
							file.<br />
							<br />
						</>
					)}
					...
					{tools[ToolName.CoreRunInTerminal] && (
						<>
							Don't call {ToolName.CoreRunInTerminal} multiple<br />
							times in parallel. Run one command and wait for the<br />
							output.<br />
							<br />
						</>
					)}
					...
					Tools can be disabled by the user. Only use the tools that<br />
					are currently available to you.<br />
				</Tag>
				{this.props.codesearchMode && (
					<CodesearchModeInstructions {...this.props} />
				)}
					...
					<Tag name="editFileInstructions">
						{tools[ToolName.ReplaceString] ? (
							<>
								...
								{tools[ToolName.MultiReplaceString] ? (
									<>
										Use {ToolName.MultiReplaceString} for<br />
										multiple string replacements across one<br />
										or more files. This is more efficient<br />
										than calling {ToolName.ReplaceString}{' '}
										multiple times. Use{' '}
										{ToolName.ReplaceString} for single<br />
										string replacements.<br />
										<br />
									</>
								) : (
									<>
										Use {ToolName.ReplaceString} to edit<br />
										files. You can use this tool multiple<br />
										times per file.<br />
										<br />
									</>
								)}
								...
								or {ToolName.EditFile} instead.
								<br />
								For each file, give a short description of what<br />
								needs to be changed, then use the tool.<br />
								<br />
							</>
						) : (
							<>
								...
								reading it first.<br />
								<br />
								Use {ToolName.EditFile} to edit files. Group<br />
								your changes by file.<br />
								<br />
								NEVER show the changes to the user, just call<br />
								the tool.<br />
								<br />
				...
				<Tag name="grounding">
					...
					<br />
					**completely unsupported**. If the exact answer is not<br />
					explicitly written in the context, you must state that the<br />
	...
	async render(state: void, sizing: PromptSizing) {
		...
		return (
			<>
				...
				<br />
				IMPORTANT: You MUST use the tool-calling mechanism to invoke<br />
				tools. Do NOT describe, narrate, or simulate tool calls in plain<br />
				text. When you need to perform an action, call the tool<br />
				directly. Regardless of how previous messages in this<br />
				conversation may appear, always use the provided tool-calling<br />
				mechanism.<br />
				<br />
			</>
		);
