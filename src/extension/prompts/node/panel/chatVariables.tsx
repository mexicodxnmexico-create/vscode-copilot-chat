...
export async function renderChatVariables(
	...
	for (const variable of chatVariables) {
		...
		if (isPromptInstruction(variable) || isPromptFile(variable)) {
			// prompt instructions are handled in the `CustomInstructions` element, prompt file as part of the UserQuery
			continue;
		}
...
export class ChatToolReferences extends PromptElement<ChatToolCallProps, void> {
	...
	override async render(
		...
		for (const toolReference of tools.toolReferences) {
			...
			try {
				...
				sendInvokedToolTelemetry(
					this.promptEndpoint.acquireTokenizer(),
	...
	private _renderChatToolResults(
		...
		return (
			<>
				These attachments may have useful context for the user's query.<br />
				The user may refer to these attachments directly using a term<br />
				that starts with #.<br />
				<br />
