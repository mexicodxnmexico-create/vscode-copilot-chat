	...
	async render(state: void, sizing: PromptSizing) {
		...
		const instructions = (
			<InstructionMessage priority={900}>
				...
				1. Please come up with a solution that you first describe<br />
				step-by-step.<br />
				<br />
				2. Group your changes by file. Use the file path as the header.
				<br />
				3. For each file, give a short summary of what needs to be<br />
				changed followed by a code block that contains the code changes.<br />
				<br />
				4. The code block should start with four backticks followed by<br />
				the language.<br />
				<br />
				5. On the first line of the code block add a comment containing<br />
				the filepath. This includes Markdown code blocks.<br />
				<br />
				6. Use a single code block per file that needs to be modified,<br />
				even if there are multiple changes for a file.<br />
				<br />
				7. The user is very smart and can understand how to merge your<br />
				code blocks into their files, you just need to provide minimal<br />
				hints.<br />
				<br />
				8. Avoid repeating existing code, instead use comments to<br />
				represent regions of unchanged code. The user prefers that you<br />
				...
				Here is an example of how you should format a code block<br />
				belonging to the file example.ts in your response:<br />
				<br />
				<Tag name="example">
					###{' '}
					...
					<br />
					<br />
					Add a new property 'age' and a new method 'getAge' to the<br />
					class Person.<br />
					<br />
					<br />
					<ExampleCodeBlock
	...
	override render() {
		...
		return (
			<TextChunk>
				{'<fileRestrictions>'}
				<br />
				The following files are readonly. Making edits to any of these<br />
				file paths is FORBIDDEN. If you cannot accomplish the task<br />
				without editing the files, briefly explain why, but NEVER edit<br />
				any of these files:<br />
				<br />
				{readonlyUris
					.map(
						(uri) =>
			...
			<>
				<UserMessage>
					...
					<Tag name="reminder" flexGrow={2} priority={899}>
						...
						comment with `{EXISTING_CODE_MARKER}` to represent<br />
						regions of unchanged code.<br />
						<br />
						The code block for each file being edited must start<br />
						with a comment containing the filepath. This includes<br />
						Markdown code blocks.<br />
						<br />
						For existing files, make sure the filepath exactly<br />
						matches the filepath of the original file.<br />
						<br />
						<NotebookReminderInstructions
							chatVariables={chatVariables}
							query={query}
...
export class WorkingSet extends PromptElement<WorkingSetPromptProps> {
	public override render(state: void, sizing: PromptSizing) {
		...
		return workingSet.length ? (
			<>
				The user has provided the following files as input. Always make<br />
				changes to these files unless the user asks to create a new<br />
				file.<br />
				<br />
				Untitled files are files that are not yet named. Make changes to<br />
				them like regular files.<br />
				<br />
...
export class NewFilesLocationHint extends PromptElement {
	...
	public override render(state: void, sizing: PromptSizing) {
		...
		if (workspaceFolders.length === 1) {
			return (
				<>
					When suggesting to create new files, pick a location inside<br />
					`<br />
