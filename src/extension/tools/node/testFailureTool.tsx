...
export class TestFailureTool implements ICopilotTool<{}> {
	...
	async invoke({
		tokenizationOptions,
	}: LanguageModelToolInvocationOptions<{}>): Promise<LanguageModelToolResult> {
		const failures = Array.from(this.testProvider.getAllFailures());
		if (failures.length === 0) {
			return new LanguageModelToolResult([
				new LanguageModelTextPart(
					`No test failures were found yet, call the tool ${ToolName.CoreRunTest} to run tests and find failures.`,
				),
			]);
		}
		...
		const json = await renderPromptElementJSON(
			...
			tokenizationOptions && {
				...tokenizationOptions,
				tokenBudget:
					tokenizationOptions.tokenBudget *
					Constant.IdealMaxTokenUsageProportion,
			},
		);
...
export class TestFailureList extends PromptElement<TestFailureListElementProps> {
		...
		return (
			<>
				<PrioritizedList priority={100} descending>
					{this.sortByRanks(this.props.failures).map((f) => (
						<TestFailureElement failure={f} />
					))}
				</PrioritizedList>
				<TextChunk priority={101}>
					## Rules:
					<br />
					- Always try to find an error in the implementation code<br />
					first. Don't suggest any changes in my test cases unless I<br />
					tell you to.<br />
					<br />- If you need more information about anything in the
					codebase, use a tool like {ToolName.ReadFile},{' '}
					{ToolName.ListDirectory}, or {ToolName.FindFiles} to find<br />
					and read it. Never ask the user to provide it themselves.<br />
					<br />- If you make changes to fix the test, call{' '}
					{ToolName.CoreRunTest} to run the tests and verify the fix.
					<br />
					- Don't try to make the same changes you made before to fix<br />
					the test. If you're stuck, ask the user for pointers.<br />
					<br />
				</TextChunk>
			</>
		);
	...
	private sortByRanks(failures: readonly ITestFailure[]) {
		const withRanks = failures.map((failure) => {
			let rank = failure.snapshot.uri
				? this.rankFile(failure.snapshot.uri)
				: Constant.RankNone;
	...
	private rankFile(uri: URI): number {
		if (
			this.tabsAndEditorsService.activeTextEditor?.document.uri.toString() ===
			uri.toString()
		) {
		...
		if (
			this.tabsAndEditorsService.visibleTextEditors?.some(
		...
		const repository = this.gitExtensionService
			.getExtensionApi()
			?.getRepository(uri);
		if (repository) {
...
class TestFailureElement extends PromptElement<TestFailureElementProps> {
	constructor(props: TestFailureElementProps) {
		super(props);
	}
	...
	render() {
		...
		for (
			let n: TestResultSnapshot | undefined = f.snapshot;
			n;
			n = n.parent
		) {
		...
		return (
			<>
				...
				>
					{f.task.messages.map((m) => (
						<TestMessageElement message={m} />
					))}
				</Tag>
			</>
		);
				...
				if (position && uri) {
					children.push(
						<Tag
							name="stackFrame"
							attrs={{
								path: this.workspaceService.asRelativePath(uri),
								line: position.line,
					...
					children.push(
						<Tag
							...
							attrs={{
								path:
									uri &&
									this.workspaceService.asRelativePath(uri),
								line: position?.line,
								col: position?.character,
							}}
						>
