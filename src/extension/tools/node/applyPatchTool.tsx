...
export class ApplyPatchTool implements ICopilotTool<IApplyPatchToolParams> {
	...
	async prepareInvocation(
		...
		const uris = [
			...identify_files_needed(options.input.input),
			...identify_files_added(options.input.input),
		].map((f) => URI.file(f));
...
> {
	override render(): PromptPiece | undefined {
		return (
			<>
				<SystemMessage>
					You are an expert in file editing. The user has provided a<br />
					patch that failed to apply because it references context<br />
					that was not found precisely in the file. Your task is to<br />
					fix the patch so it can be applied successfully.<br />
					<Tag name="patchFormat">
						The expected format for the patch is a diff format that<br />
						modifications and include contextual lines around the<br />
						changes. The patch should be formatted as follows:<br />
						<br />
						<ApplyPatchFormatInstructions />
						The output MUST NOT actually include the string "[3<br />
						lines of pre-context]" or "[3 lines of post-context]" --<br />
						include the actual lines of context from the file. An<br />
						example of a patch you might generate is shown below.<br />
						<br />
						<br />
						```
						<br />
					...
					<Tag name="instructions">
						1. Think carefully. Examine the provided patch, the<br />
						included intent, the contents of the files it<br />
						references.<br />
						<br />
						2. Determine the locations in the files where the user<br />
						intended the patch to be applied. Lines that don't begin<br />
						with a plus "+" or "-" sign must be found verbatim in<br />
						the original file, and ONLY lines to be added or removed<br />
						should begin with a plus or minus sign respectively. It<br />
						is very likely this rule is being broken by the invalid<br />
						patch.<br />
						<br />
						3. Generate the ENTIRE corrected patch. Do not omit<br />
						anything.<br />
						<br />
					</Tag>
				</SystemMessage>
