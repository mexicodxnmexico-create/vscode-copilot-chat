...
export class RunNotebookCellOutput extends PromptElement<IRunNotebookCellOutputProps> {
	override render(state: void, sizing: PromptSizing) {
		...
		if (textItem) {
			if (
				getCharLimit(textItem.data.byteLength) >
				sizing.tokenBudget / this.props.sizeLimitRatio
			) {
				return (
					<Tag
						name={`cell-output`}
					...
					>
						Output {index + 1}: Output is too large to be used as<br />
						context in the language model, but the user should be<br />
						able to see it in the notebook.<br />
						<br />
						<br />
					</Tag>
				);
		...
		const largeOutput = output.items.find(
			(item) =>
				getCharLimit(item.data.byteLength) >
				sizing.tokenBudget / this.props.sizeLimitRatio,
		);
		if (largeOutput) {
			return (
				...
				>
					Output {index + 1}: Output is too large to be used as<br />
					context in the language model, but the user should be able<br />
					to see it in the notebook.<br />
					<br />
					<br />
				</Tag>
			);
