...
export async function findAllReferencedFunctionImplementationsInSelection(
	...
	const functionDefinitionsCache = new Map<string, TreeSitterExpressionInfo[]>();
		...
		for (const link of impl) {
			...
			if (treeSitterAST) {
				...
				if (!functionDefinitions) {
					functionDefinitions = await treeSitterAST.getFunctionDefinitions(); // Cached per document
...
export async function findAllReferencedClassDeclarationsInSelection(
	...
	const classDeclarationsCache = new Map<string, TreeSitterExpressionInfo[]>();
	for (let i = 0; i < implementations.length; i++) {
		...
		for (const link of impl) {
			...
			const treeSitterAST = parserService.getTreeSitterAST(textDocument);
			if (treeSitterAST) {
				let classDeclarationsForDoc = classDeclarationsCache.get(uri.toString());
				if (!classDeclarationsForDoc) {
					classDeclarationsForDoc = await treeSitterAST.getClassDeclarations(); // Cached per document
					classDeclarationsCache.set(uri.toString(), classDeclarationsForDoc);
				}
				const classDeclaration = classDeclarationsForDoc.find((fn) => fn.identifier === match.identifier);
				if (classDeclaration) {
					const treeSitterRange = vscodeToTreeSitterOffsetRange(range, textDocument);
