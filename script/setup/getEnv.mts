...
function red(s: string) {
	return useColors ? `\x1b[31m${s}\x1b[0m` : s;
}
...
async function fetchSecrets(): Promise<{ [key: string]: string | undefined }> {
	const keyVaultClient = await setupSecretClient(
		'https://copilot-automation.vault.azure.net/',
	);
	...
	if (!process.stdin.isTTY) {
		// only in automation
		secrets['GITHUB_OAUTH_TOKEN'] = await fetchSecret(
			keyVaultClient,
			'capi-oauth',
		);
		secrets['VSCODE_COPILOT_CHAT_TOKEN'] = await fetchSecret(
			keyVaultClient,
			'copilot-token',
		);
		secrets['GHCR_PAT'] = await fetchSecret(keyVaultClient, 'ghcr-pat');
		secrets['BLACKBIRD_EMBEDDINGS_KEY'] = await fetchSecret(
			keyVaultClient,
			'vsc-aoai-key',
		);
		secrets['BLACKBIRD_REDIS_CACHE_KEY'] = await fetchSecret(
			keyVaultClient,
			'blackbird-redis-cache-key',
		);

		try {
			secrets['ANTHROPIC_API_KEY'] = await fetchSecret(
				keyVaultClient,
				'anthropic-key',
			);
			secrets['DEEPSEEK_API_KEY'] = await fetchSecret(
				keyVaultClient,
				'deepseek-key',
			);
		} catch (error) {
			console.log(
				red(`Failed to fetch optional evaluation tokens. Skipping...`),
			);
		}
	}
